...
namespace reactor
{
    namespace detail
    {
        ...
        public:
            ThreadNameInitializer()
            {
                ...
                CurrentThread::t_threadName = "main";
                CurrentThread::tid();
                pthread_atfork(NULL, NULL, &afterfork);//给fork后的子进程设置初始化
            }
        };
        ThreadNameInitializer init;//给父进程初始化，子进程从fork后开始运行，不会运行这段代码
        ...
        {
            struct ThreadData *data = static_cast<struct ThreadData *>(obj);
            data->runInThread();
            delete data;
            return nullptr;
        }
    ...
    void Thread::start()
    {
        assert(!started_);
        started_ = true;
        struct detail::ThreadData *data = new struct detail::ThreadData(func_, name_, &tid_, &latch_);
